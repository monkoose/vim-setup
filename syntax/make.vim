...
" identifiers
syn region makeIdent	start="\$(" skip="\\)\|\\\\" end=")" contains=makeIdent
syn region makeIdent	start="\${" skip="\\}\|\\\\" end="}" contains=makeIdent
...
syn match makeIdentOpen containedin=makeIdent "(" nextgroup=makeStatement transparent
syn keyword makeStatement contained abspah addprefix addsuffix and basename call dir error eval file
syn keyword makeStatement contained filter-out filter findstring firstword flavor foreach guile if info
syn keyword makeStatement contained join lastword notdir or origin patsubst realpath shell sort strip subst suffix
syn keyword makeStatement contained value warning wildcard word wordlist words
